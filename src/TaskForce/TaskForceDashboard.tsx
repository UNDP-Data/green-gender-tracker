...
export const TaskForceDashboard = (props: Props) => {
  ...
  const tfsByRegionWomenMajority = filteredTFs.filter((d) => d['Composition Classification'] === 'Majority Women' || d['Composition Classification'] === 'Gender Parity');
  ...
  const tfMemberTotalMultiSectoral = sumBy(filteredTFs.filter((d) => d['Sector 2'] === 'Multi-Sectoral' && d['Composition Data'] === 'Yes'), (d) => {
    const men = d['#Men'] ? d['#Men'] : 0;
    const women = d['#Women'] ? d['#Women'] : 0;
    return men + women;
  });
  const tfMemberWomenMultiSectoral = sumBy(filteredTFs.filter((d) => d['Sector 2'] === 'Multi-Sectoral' && d['Composition Data'] === 'Yes'), (d) => (d['#Women'] ? d['#Women'] : 0));

  const tfMemberTotalPublicHealth = sumBy(filteredTFs.filter((d) => d['Sector 2'] === 'Public Health' && d['Composition Data'] === 'Yes'), (d) => {
    const men = d['#Men'] ? d['#Men'] : 0;
    const women = d['#Women'] ? d['#Women'] : 0;
    return men + women;
  });
  const tfMemberWomenPublicHealth = sumBy(filteredTFs.filter((d) => d['Sector 2'] === 'Public Health' && d['Composition Data'] === 'Yes'), (d) => (d['#Women'] ? d['#Women'] : 0));
    ...
    <div className='margin-top-09'>
      <div className='flex-div margin-bottom-05 flex-space-between flex-wrap'>
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          ...
          <p className='margin-top-05 margin-bottom-00 undp-typography'>No. of task torces</p>
        </StatCardsDiv>
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          <h3 className='margin-bottom-00 undp-typography'>{filteredCountryData.length}</h3>
          <p className='margin-top-05 margin-bottom-00 undp-typography'>No. of countries with task force data</p>
        </StatCardsDiv>
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          <h3 className='margin-bottom-00 undp-typography'>{filteredTFs.filter((d) => d['Composition Data'] === 'Yes').length}</h3>
          <p className='margin-top-05 margin-bottom-00 undp-typography'>No. of task forces with membership data</p>
        </StatCardsDiv>
      </div>
      <div className='flex-div margin-top-07 flex-space-between flex-wrap'>
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          <h3 className='margin-bottom-00 undp-typography'>{tfsByRegionWomenMajority.length}</h3>
        ...
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          <h3 className='margin-bottom-00 undp-typography'>
            {((tfsByRegionWomenLeader.length * 100) / filteredTFs.length).toFixed(1)}
            %
          </h3>
          <p className='margin-top-05 margin-bottom-00 undp-typography'>Share of task force with women leader or co-chair</p>
        </StatCardsDiv>
        <StatCardsDiv className='stat-card' width='calc(33.33% - 1.334rem)'>
          <h3 className='margin-bottom-00 undp-typography'>
            {(tfMemberAvgPercent).toFixed(1)}
            %
          </h3>
